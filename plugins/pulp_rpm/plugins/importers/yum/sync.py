...
class RepoSync(object):
    ...
    def sync_feed(self):
        """
        :return:    list of URLs of the feed we should sync
        :rtype:     list
        """
        mirrorlist = self.call_config.get(importer_constants.KEY_MIRRORLIST)
        repo_url = self.call_config.get(importer_constants.KEY_FEED)
        if mirrorlist is not None:
            # Parse the mirrorlist url and return a list of base url
            response = urllib2.urlopen(mirrorlist)
            url_parse = response.read().split('\n')
            repo_url = []
            pattern = re.compile("^[^#]*((http|https|ftp)://.*)")
            for line in url_parse:
                for match in re.finditer(pattern, line):
                    repo_url.append(match.groups()[0])
            return repo_url
        ...
        for url in self.sync_feed:
            ...
            try:
                with self.update_state(self.progress_status['metadata']):
                    metadata_files = self.get_metadata(url)
                ...
                if url_count == len(self.sync_feed):
            ...
            return self.sync_conduit.build_success_report(
